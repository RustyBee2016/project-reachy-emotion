**Detailed_Implementation_Guide_08.4.2_Sonnet_4.5**

**Reachy_Local_08.4.2 Detailed Implementation**

**Guide**

**Date**: 2025-11-03 **Version**: 1.0 **Status**: Ready for Review

**Executive Summary**

This document provides detailed implementation specifications for
completing

Reachy_Local_08.4.2. Each section includes production-ready code with
comprehensive logic explanations. **Do not implement yet** - this is for
review and

approval first.

**Implementation Approach**: Test-driven, incremental delivery with
validation
...
**Core Logic**: Exponential backoff with jitter prevents thundering herd
problem.

Classifies errors as retryable (5xx, network) vs non-retryable (4xx).
Idempotency
...
key =
hashlib.sha256(f\"{video_id}:{dest_split}:{time.time()//60}\".encode()).hexdigest()
...
json={\"video_id\": video_id, \"dest_split\": dest_split, \"label\":
label}
...
**Core Logic**: Maintains persistent connection with auto-reconnect.
Messages

queued for Streamlit polling. Event handlers dispatch to callbacks.
...
st.session_state.ws_manager.get_messages():
...
**Core Logic**: training_run tracks experiments with run_id.
training_selection

links videos to runs with constraints preventing duplicates. Triggers
auto-update
...
train_fraction NUMERIC(3,2) **CHECK** (train_fraction \> 0 **AND**
train_fraction \< 1),

test_fraction NUMERIC(3,2) **CHECK** (test_fraction \> 0 **AND**
test_fraction \< 1),
...
**CONSTRAINT** valid_fractions **CHECK** (train_fraction + test_fraction
\<= 1.0)
...
target_split VARCHAR(20) **CHECK** (target_split **IN**
(\'train\',\'test\')),
...
**Core Logic**: Docker compose orchestrates TAO 4.x (training) and TAO
5.3

(export) containers. Volume mounts provide access to dataset and models.
GPU

passthrough enables CUDA acceleration.
...
**Core Logic**: Queries database for balanced sample using stored
procedure.

Copies files from dataset_all to train/test using run_id. Generates
JSONL

6

manifests for TAO. Updates training_selection table.
...
INSERT INTO training_run (run_id, strategy, train_fraction,
test_fraction, seed, sta
...
dst_dir = base_path / target_split / label **if** target_split ==
\'train\' **else** base_path
...
entry = {\"video_id\": str(video_id), \"path\": str(dst), \"label\":
label **if** target_split
...
**with** open(manifest_dir / f\"{run_id}\_train.jsonl\", \'w\') **as**
f:
...
cur.execute(\"UPDATE training_run SET status = \'ready\' WHERE run_id =
%s\", (run_id,))
...
**Core Logic**: Wraps TAO training with MLflow tracking. Logs
hyperparameters,

metrics per epoch, artifacts (models, confusion matrices). Links
dataset_hash
...
cur.execute(\"SELECT dataset_hash FROM training_run WHERE run_id = %s\",
(run_id,))
...
**Core Logic**: GStreamer pipeline with nvinfer element for TensorRT
inference.

Preprocessing normalization, batching for efficiency, postprocessing for
class
...
**Core Logic**: Uses TAO 5.3 to convert trained model to TensorRT
engine.

FP16 precision for speed/memory balance. INT8 calibration optional for
further
...
OUTPUT_PATH=\"/workspace/engines/emotion\_\${RUN_ID}\_fp16.engine\"

*\# Export with TAO 5.3*

docker-compose -f docker-compose-tao.yml exec tao-export tao emotionnet
export \\
...
**Core Logic**: Connects to Ubuntu 2 gateway via WebSocket. Sends
emotion events with device_id, timestamp, inference metrics. Receives
cues for
...
**def** \_\_init\_\_(self, gateway_url, device_id):
...
**def** send_emotion(self, emotion: str, confidence: float,
inference_ms: float):
...
**Core Logic**: Triggered by webhook or file upload. Computes SHA256
hash,

extracts metadata with ffprobe, generates thumbnail, writes to DB, emits
completion event.
...
\"command\": \"ffmpeg -i {{\$json.file_path}} -vframes 1 -vf
scale=320:-1 /thumbs/{{\$jso
...
\"columns\":
\"video_id,file_path,sha256,duration_sec,width,height,size_bytes\"
...
**Core Logic**: Polls database for dataset readiness (balanced,
sufficient samples).

Triggers dataset preparation, launches TAO training, monitors progress,
validates

gates, exports TensorRT engine.
...
\"command\": \"python /workspace/trainer/prepare_dataset.py \--run-id
{{\$json.run_id}}\"
...
\"command\": \"docker-compose -f docker-compose-tao.yml exec tao-train
tao emotionnet t
...
\"query\": \"SELECT metrics-\>\>\'f1_macro\' as f1 FROM training_run
WHERE run_id = \'{{\$jso
...
\"command\": \"/workspace/trainer/tao/export_to_tensorrt.sh
{{\$json.run_id}}\"
...
**Key Metrics**: - API latency (p50, p95, p99) - Error rate by
endpoint - Database

connection pool utilization - Video processing queue depth - Training
run status

\- Jetson inference FPS and latency - GPU utilization and temperature
...
**Core Logic**: Middleware validates Bearer tokens. Uses RS256
asymmetric

encryption. Tokens expire after 15 minutes. Refresh tokens valid for 7
days.
...
**Next Steps**: Review this plan, provide feedback, approve for
implementation.
