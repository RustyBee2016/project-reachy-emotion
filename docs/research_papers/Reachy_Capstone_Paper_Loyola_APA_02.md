...
Ten n8n agents: Ingest, Labeling, Promotion, Reconciler, Training Orchestrator, Evaluation, Deployment, Privacy, Observability, and Gesture.
...
***Test set reuse.*** The same 894-image test set is used across all experimental phases. Repeated evaluation on identical data risks optimistic bias if development decisions are informed by test performance. This threat is mitigated by using Gate A-val (synthetic validation) as the primary development metric; test evaluation occurs only at phase end. Nonetheless, a fully held-out test set per phase would provide stronger guarantees.

***Temperature scaling data overlap.*** The temperature parameter T was learned on a 30% calibration split (268 images) of the same test set used for evaluation. The single-parameter nature of temperature scaling minimizes overfitting risk, and the calibration improvements are large enough (ECE: 0.142 to 0.036) to be robust to sampling variability. A fully independent calibration set would be more rigorous.

***Hyperparameter search asymmetry.*** V2 underwent a 90-trial hyperparameter sweep; V1 used a single configuration. V1's simpler architecture has fewer hyperparameters to tune, partially mitigating this concern, but V2 may benefit from more thorough optimization.

## External Validity

***Single test dataset.*** AffectNet provides the only real-world evaluation. Deployment environments may present lighting, camera angles, occlusions, and demographics that differ from both synthetic and AffectNet data.

***Static image evaluation.*** The test set consists of still photographs, but the deployed system processes video with temporal dynamics not evaluated here. The temporal smoothing component (15 frames, 60% consistency) remains untested.

***Cultural and demographic coverage.*** AffectNet is internet-sourced with global diversity, but specific deployment contexts may have underrepresented demographics.

## Construct Validity

***F1 Macro equal weighting.*** Macro averaging gives equal weight to all three classes, but neutral expressions may represent ~75% of real interactions. Operational importance is not reflected in the metric.

***ECE binning.*** Ten equal-width bins with 894 samples may produce sparse bins, introducing variance in the ECE estimate. Adaptive binning or kernel density approaches would provide more stable estimates.

***Composite score weights.*** The weights (0.50, 0.20, 0.15, 0.15) were chosen heuristically based on deployment priorities, not from formal utility analysis. Different weight choices could change the composite ranking, though V2 mixed+T dominates every component.
...
***Expanded real-data sampling.*** The current mixed-domain training uses 15,000 real images. Scaling to 30,000--50,000 per class would test whether the synthetic-to-real improvement continues or plateaus. A learning curve analysis would identify the minimum real data required for gate compliance.

***Ensemble methods.*** V1 and V2 have complementary error profiles in the synthetic-only regime: V1 excels on sad/neutral, V2 on happy. An ensemble combining their predictions could outperform either variant individually, particularly in early training phases before mixed-domain data is available.

***8-class Ekman taxonomy.*** The current 3-class taxonomy (happy, sad, neutral) is Phase 1. Phase 2 expands to the 8-class Ekman taxonomy (adding anger, contempt, disgust, fear, surprise), which is already supported by the HSEmotion backbone and the database schema.

***Adversarial domain adaptation.*** While simple mixed-domain augmentation proved effective, adversarial techniques (Ganin & Lempitsky, 2015) could provide additional gains, particularly if real data acquisition is constrained.

***Fairness evaluation.*** Evaluating per-demographic-group performance (age, gender, ethnicity) would reveal whether the model exhibits disparate accuracy, an important consideration for companion robots deployed in diverse settings.

***Video-level evaluation.*** Testing on video sequences rather than static images would validate the temporal smoothing pipeline and evaluate performance on expression onset/offset dynamics rather than peak expressions only.
