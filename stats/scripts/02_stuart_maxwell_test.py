...
def generate_demo_data(
    ...
    base_accuracies = np.full(n_classes, 0.82, dtype=float)
    
    # Fine-tuned model accuracies (effect depends on effect_size)
    if effect_size == "none":
        ft_accuracies = base_accuracies.copy()
    elif effect_size == "small":
        ft_accuracies = np.clip(base_accuracies + 0.01, 0.0, 1.0)
    elif effect_size == "medium":
        ft_accuracies = np.clip(base_accuracies + 0.03, 0.0, 1.0)
    else:  # large
        ft_accuracies = np.clip(base_accuracies + 0.06, 0.0, 1.0)
    ...
    for i in range(n_samples):
        ...
        if np.random.random() < base_accuracies[true_class]:
            ...
            base_preds[i] = int(np.random.choice(alternatives))
        ...
        if np.random.random() < ft_accuracies[true_class]:
            ...
            alternatives = [idx for idx in range(n_classes) if idx != true_class]
            ft_preds[i] = int(np.random.choice(alternatives))
...
Examples:
    ...
    if args.demo:
        ...
        _configure_runtime(cli_classes or file_classes or list(DEFAULT_EMOTION_CLASSES))
