...
# Index of neutral class (for F1 Neutral calculation)
...
def plot_per_class_f1(report: MetricsReport, output_path: Optional[Path] = None) -> None:
    ...
    colors = ['#2ecc71' if v >= 0.80 else '#e74c3c' for v in f1_values]
    
    plt.figure(figsize=(12, 6))
    bars = plt.bar(classes, f1_values, color=colors, edgecolor='black')
    
    # Threshold line
    plt.axhline(y=0.80, color='red', linestyle='--', label='Threshold (0.80)')
...
def generate_demo_data(n_samples: int = 1000, seed: int = 42) -> Tuple[np.ndarray, np.ndarray]:
    ...
    # More happiness/neutral, less contempt/disgust
    class_weights = [0.10, 0.05, 0.08, 0.10, 0.22, 0.20, 0.15, 0.10]
    y_true = np.random.choice(len(EMOTION_CLASSES), size=n_samples, p=class_weights)
    
    # Generate predictions with realistic accuracy per class
    # Simulate a model that's good at happiness/neutral but struggles with contempt
    class_accuracies = [0.82, 0.70, 0.75, 0.78, 0.90, 0.88, 0.84, 0.80]
    ...
    for i in range(n_samples):
        ...
        if np.random.random() < class_accuracies[true_class]:
            ...
            # Misclassify to a similar emotion
            confusion_map = {
            ...
            y_pred[i] = np.random.choice(confusion_map[true_class])
